    ...
    {showConsent && pendingUser && (
      <div className="fixed inset-0 z-[100] flex flex-col"
        ...
        <div className="flex-shrink-0 px-5 pb-3">
          <div className="flex rounded-2xl p-1" style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }}>
            {([{ id: "terms", label: "Условия использования" }, { id: "privacy", label: "Конфиденциальность" }] as const).map(tab => (
              <button key={tab.id} onClick={() => setConsentTab(tab.id)}
        ...
        <div className="flex-1 overflow-y-auto px-5 flex flex-col gap-4 pb-4" style={{ scrollbarWidth: "none" }}>
          ...
          {consentTab === "privacy" && (<>
            ...
            {([
              ...
              { icon: "Share2", title: "3. Передача данных третьим лицам", items: ["Данные не продаются и не передаются в коммерческих целях.", "Данные передаются партнёрам-обработчикам на основании договоров о конфиденциальности.", "Данные передаются правоохранительным органам по законному запросу."] },
              { icon: "Lock", title: "4. Хранение и защита", items: ["Данные хранятся на серверах на территории РФ (ч. 5 ст. 18 ФЗ-152).", "Применяются шифрование TLS/SSL, хэширование паролей (bcrypt), контроль доступа.", "Данные хранятся в течение срока действия аккаунта и 30 дней после удаления.", "При утечке данных Компания уведомит пользователей в течение 72 часов."] },
              { icon: "UserCheck", title: "5. Права пользователей", items: ["Право на доступ: запросить копию своих данных (ст. 14 ФЗ-152).", "Право на исправление: скорректировать данные через настройки.", "Право на удаление: в течение 30 дней после запроса.", "Право на возражение: отозвать согласие, направив запрос на info@lbloom.ru."] },
              { icon: "Globe", title: "6. Международная передача", items: ["По умолчанию данные не передаются за пределы РФ.", "Пользователи из ЕС имеют дополнительные права по GDPR."] },
              { icon: "Bell", title: "7. Изменения политики", items: ["Пользователи уведомляются об изменениях за 7 дней.", "Актуальная версия всегда доступна в настройках приложения."] },
            ] as const).map(section => (
              <div key={section.title} className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
        ...
        <div className="flex-shrink-0 px-5 pt-3 pb-10 flex flex-col gap-3"
          ...
          <button
            onClick={() => { setShowConsent(false); if (pendingUser) onAuth(pendingUser); }}
