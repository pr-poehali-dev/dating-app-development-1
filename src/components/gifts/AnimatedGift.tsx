...
// Тема фона под каждую категорию
const BG_THEME: Record<GiftSceneCategory, [string, string]> = {
  heart:   ["#8B3A52", "#4A1D2E"],
  rose:    ["#7A3548", "#3D1A26"],
  bear:    ["#7A5A3A", "#3D2A1A"],
  ring:    ["#3A5A7A", "#1A2A3D"],
  special: ["#8B4A52", "#5A2A33"],
};
...
export default function AnimatedGift({
  ...
  const [themeFrom, themeTo] = BG_THEME[category] || BG_THEME.special;
  ...
  return (
    ...
    >
      ...
      {withBackground && (
        <>
          <div style={{ position: "absolute", inset: 0, background: `radial-gradient(circle at 50% 45%, ${gradFrom} 0%, ${gradTo} 100%)` }} />
          <div style={{ position: "absolute", inset: 0, backgroundImage: `url("data:image/svg+xml,${pattern}")`, backgroundSize: `${size * 0.3}px ${size * 0.3}px`, opacity: 0.45 }} />
