...
def handler(event: dict, context) -> dict:
    ...
    cur.execute(f"""
        ...
        RETURNING id, order_number, user_email, user_id, plan
    """, (int(inv_id),))
    ...
    order_id, order_number, user_email, user_id, plan = result

    # Активируем Premium пользователю
    if user_id:
        cur.execute(f"UPDATE {schema}.users SET premium = true WHERE id = %s", (user_id,))
