"""
Лайки: поставить лайк/дизлайк, получить входящие лайки, проверить матч
"""
import json
import os
import psycopg2
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if method == 'POST' and path.endswith('/likes'):
            ...
            if not to_id:
                return {'statusCode': 400, 'headers': CORS, 'body': json.dumps({'error': 'Укажи to_user_id'})}
            ...
            conn.commit()
            return {'statusCode': 200, 'headers': CORS, 'body': json.dumps({'ok': True, 'match': bool(mutual), 'match_id': match_id})}

        # GET /likes — входящие лайки (кто лайкнул меня)
        if method == 'GET' and path.endswith('/likes'):
            ...
            for i, r in enumerate(rows):
                ...
                # Скрываем для не-премиум (оставляем только первый)
                item['blurred'] = not me['premium'] and i > 0
                liked_me.append(item)
            return {'statusCode': 200, 'headers': CORS, 'body': json.dumps({'liked_me': liked_me, 'total': len(liked_me)})}
